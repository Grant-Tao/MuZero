...
class Prediction(nn.Module):
    ...
    def forward(self, rp):
        ...
        h_v = self.fc_v1(h_v.view(-1, self.board_size))
        h_v = self.fc_v2(h_v.view(-1, 256))
...
class Dynamics(nn.Module):
    ...
    def forward(self, rp, a):
        a = a.reshape(a.shape[0], 1, a.shape[1], a.shape[2])
...
def gen_batch(games):
    ...
    for i in range(batch_size): 
        ...
        state0 = np.zeros((18, 361), dtype=np.uint8)
        actions = np.zeros((FK, 361), dtype=np.uint8)
        probs = np.zeros((FK+1, 362), dtype=np.float32)
        winners = np.zeros((FK+1), dtype=np.float32)
        for j in range(8) :
            k = s0 - j
            if k < 0:
                k=0
            state0[2*j] = game[k][0]
            state0[2*j+1] = game[k][1]
            stm = game[s0][2]
            if stm == 0:  #black to move
                state0[16] = 1
            else :
                state0[17] = 1
        state0 = state0.reshape(18, 19, 19)
        state0 = state0.astype(np.float32)
        for j in range(FK+1) :
            probs[j] = game[s0+j][4]
            winners[j]   = game[s0+j][5]
            if j < FK :
                action = game[s0+j][3]
                if action < 361 :
                    actions[j][action] = 1
        actions = actions.reshape(FK, 19, 19)
...
def train(games, nets=Nets()):
    ...
    for epoch in range(num_epochs):
        ...
        for _ in range(num_steps) :
            ...
            x, ax, p_target, v_target = zip(*[batchdata[i] for i in range(batch_size)])
            ...
            for t in range(FK + 1):
                ...
                print(p_loss.eval(), v_loss.eval())                
                #print(p_target[t], v_target[t])
...
with gzip.open(filename, 'rb') as fp :
    games = cPickle.load(fp)

nets = Nets()

nets = train(games, nets)
